...
(defn create-user
  ...
  (let [user-return 
        (insert users
                (values [{:username username :email email :password_digest pass_digest :created_at (sqlfn now) :updated_at (sqlfn now) }]))]
...
(defn create-bookmark
  ...
  (let [book-return 
        (insert bookmarks
                (values [{:user_id user-id :title title :desc description :url address :created_at (sqlfn now) :updated_at (sqlfn now) }]))]
